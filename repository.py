...
class SqliteRepository:
    ...
    def get_all_test_id_subject(self, subject: str):
        ...
        return f'All test id for {subject}: {id_subject}'
    ...
    def get_result_user(self, user_id: int):
        try:
            ...
            for row in data:
                ...
                results.append([result.test_id, result.score])
            return f'UserID - {user_id} results testID/score: {results}'
        except IndexError:
            print('User not exist')
    ...
    def add_new_result(self, test_id, user_id, score):
        ...
        '''
        cursor = self.connection.cursor()
        new_result_data = Result(int(test_id), int(user_id), int(score))
